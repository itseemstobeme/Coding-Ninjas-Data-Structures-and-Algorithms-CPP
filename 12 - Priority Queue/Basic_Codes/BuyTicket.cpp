...
{
    priority_queue<int> pq; //storing actual values of arr, ie, priority
    queue<int> q;           //stores position of elements in same order as arr
    int time = 0;
    for (int i = 0; i < n; ++i)
    {
        q.push(i);       //pushing position
        pq.push(arr[i]); //pushing value at i
    }

    while (!pq.empty())
    {
        if (arr[q.front()] == pq.top()) //If top() of PQ is same as q.front() position in arr[]
        {
            ++time;
            if (q.front() == k) //If the q.front() position is same as k(our position in array)
                return time;

            q.pop(); //else, pop both top and front from PQ and q respectively
            pq.pop();
        }
        else
        {
            q.push(q.front()); //If top() of PQ isn't same as q.front() position in arr[]
            q.pop();           // remove front() from q and add it into back as a higher priority is already in queue
        }
    }
    return time;
}
