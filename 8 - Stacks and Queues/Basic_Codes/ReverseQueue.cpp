...
#include<queue>
using namespace std;
//We use stack, but not explicitly, but implicitly using recursion
void  reverseQueue(queue<int>& input)
{   
    if(input.empty())   //If queue is empty, return 
    return;

    int front=input.front();       //Using recursion, we manipulate internal recursion stack to get our job done
    input.pop();
    reverseQueue(input);
    input.push(front);
