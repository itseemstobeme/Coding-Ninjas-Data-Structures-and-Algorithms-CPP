...
int* stockSpan(int* price, int size) 
{
    ...
    stack<pair<int,int>> s;     //first->stock price, second->its stock span

    for(int i=0;i<size;++i)
    {   
        int count=1;
        if(!s.empty())          //Always make sure we check this before accessing top()
        {
            while(s.top().first < price[i])
            {
                count+=s.top().second;
                s.pop();
                if(s.empty())       //if after pop(), stack gets empty, break immediately
                break;
            }
        }
        pair<int,int> p(price[i],count);
        s.push(p);
        span[i]=count;
    }
    return span;
